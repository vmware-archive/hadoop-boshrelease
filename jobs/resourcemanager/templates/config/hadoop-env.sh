...
# export JSVC_HOME=${JSVC_HOME:-/usr/libexec/bigtop-utils}
...
export HADOOP_NAMENODE_OPTS="-server -XX:ParallelGCThreads=8 -XX:+UseConcMarkSweepGC -XX:ErrorFile=$HADOOP_LOG_DIR/hs_err_pid%p.log -XX:NewSize=200m -XX:MaxNewSize=640m -Xloggc:$HADOOP_LOG_DIR/gc.log-`date +'%Y%m%d%H%M'` -verbose:gc -XX:+PrintGCDetails -XX:+PrintGCTimeStamps -XX:+PrintGCDateStamps -Xms1024m -Xmx1024m -Dhadoop.security.logger=INFO,DRFAS -Dhdfs.audit.logger=INFO,DRFAAUDIT ${HADOOP_NAMENODE_OPTS}"
...
export HADOOP_LOG_DIR=${HADOOP_LOG_DIR:-/var/log/hadoop/$USER}

# History server logs
export HADOOP_MAPRED_LOG_DIR=${HADOOP_MAPRED_LOG_DIR:-/var/log/hadoop-mapreduce/$USER}

# Where log files are stored in the secure data environment.
export HADOOP_SECURE_DN_LOG_DIR=${HADOOP_SECURE_DN_LOG_DIR:-/var/log/hadoop/$HADOOP_SECURE_DN_USER}
...
export HADOOP_PID_DIR=${HADOOP_PID_DIR:-/var/run/hadoop/$USER}
export HADOOP_SECURE_DN_PID_DIR=${HADOOP_SECURE_DN_PID_DIR:-/var/run/hadoop/$HADOOP_SECURE_DN_USER}

# History server pid
export HADOOP_MAPRED_PID_DIR=${HADOOP_MAPRED_PID_DIR:-/var/run/hadoop-mapreduce/$USER}
